...
### Custom Devices
TorchTune supports finetuning on a variety of devices, including NVIDIA CUDA, Intel XPU, AMD ROCm, and more. If you're interested in running recipes on a custom device, such as Intel XPU, follow the steps below.
...
Step 2: Update device information via **Config Overrides**,
You can directly overwrite config fields from the command line:

```bash
tune run lora_finetune_single_device --config llama2/7B_lora_single_device device=xpu
```
Or edit your local copy of configeration files and replace `device: cuda` using `device: xpu`

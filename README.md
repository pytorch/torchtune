...
| Model                                         | Sizes     |
|-----------------------------------------------|-----------|
| [Llama3](https://llama.meta.com/llama3)    | 8B-Instruct, 70B-Instruct [[models](torchtune/models/llama3/_model_builders.py), [configs](recipes/configs/llama3/)]        |
...
**Note**: our Llama3 configs default to the instruct fine-tuned models.
This is because not all special token embeddings are initialized in the base 8B and 70B models.

In our initial experiments for Llama3-8B-Instruct, QLoRA has a peak allocated memory of ``~9GB`` while LoRA on a single GPU has a peak allocated memory of ``~19GB``. To get started, you can use our default configs to kick off training.

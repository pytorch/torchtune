...
CONFIG="${CONFIG:-/workspace/torchtune/recipes/configs/llama3_2_vision/11B_qlora.yaml}"
MODEL_DIR="${MODEL_DIR:-./models/Llama-3.2-11B-Vision-Instruct}"
...
if [ ! -d $MODEL_DIR ]; then
    echo "11B model not found in $MODEL_DIR" >&2
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-3.2-11B-Vision-Instruct --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi

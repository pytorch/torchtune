...
CONFIG="${CONFIG:-/workspace/torchtune/recipes/configs/llama2/7B_qlora.yaml}"
MODEL_DIR="${MODEL_DIR:-./models/Llama-2-7b-hf}"
...
if [ ! -d $MODEL_DIR ]; then
    echo "7B model not found in $MODEL_DIR" >&2
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-2-7b-hf --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi

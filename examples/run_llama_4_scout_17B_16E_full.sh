...
CONFIG="${CONFIG:-/workspace/torchtune/recipes/configs/llama4/scout_17B_16E_full.yaml}"
MODEL_DIR="${MODEL_DIR:-./models/Llama-4-Scout-17B-16E-Instruct}"
...
if [ ! -d $MODEL_DIR ]; then
    echo "17B-16E model not found in $MODEL_DIR" >&2
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-4-Scout-17B-16E-Instruct --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi

...
CONFIG="${CONFIG:-/workspace/torchtune/recipes/configs/llama2/70B_qlora.yaml}"
MODEL_DIR="${MODEL_DIR:-./models/Llama-2-70b-hf}"
...
if [ ! -d $MODEL_DIR ]; then
    ...
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-2-70b-hf --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi

...
CONFIG="${CONFIG:-/workspace/torchtune/recipes/configs/llama3_2/1B_full.yaml}"
MODEL_DIR="${MODEL_DIR:-./models/Llama-3.2-1B-Instruct}"
...
if [ ! -d $MODEL_DIR ]; then
    echo "1B model not found in $MODEL_DIR" >&2
    echo "Download with: "
    echo "  huggingface-cli download meta-llama/Llama-3.2-1B-Instruct --local-dir $MODEL_DIR --exclude 'original/*.pth'"
    exit 1
fi

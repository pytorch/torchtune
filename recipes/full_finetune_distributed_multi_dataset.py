...
from torchtune.datasets._text_completion import TextCompletionTransform
...
class FullFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        #
        # Number of training steps in each epoch depends on the max_steps_per_epoch param
        # set by the user and the gradient_accumulation_steps param. This value is used
        # for logging and tracking training state. The computation should happen after
        # the dataloader has been setup
    ...
    def _setup_data(
        ...
        for idx, cfg_dataset in enumerate(cfg_datasets):
            ...
            transform = (
                config.instantiate(cfg_dataset.pop("transform"))
                if "transform" in cfg_dataset
                else lambda x: x
            )

            column = cfg_dataset.pop("column")

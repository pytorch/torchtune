...
class LoRAFinetuneRecipeDistributed(FTRecipeInterface):
    ...
    def _setup_model(
        ...
        log.info(f"FSDP is enabled. Instantiating Model on {self._device} for Rank {torch.distributed.get_rank()} ...")
        ...
        log.info(
            f"Model instantiation on rank {torch.distributed.get_rank()} took {time.perf_counter() - init_start:.2f} secs"
        )

...
class DDPFinetuneRecipeDistributed(FTRecipeInterface):
    """
    DDP finetuning recipe for dense transformer-based LLMs such as Llama2. This recipe supports
    distributed training and can be run on a single node (1 to 8 GPUs).

    Features:
        - DDP. Supported using PyTorch's DDP APIs. Training on CPU is not supported.
    ...
    def _setup_model(
        ...
        Model initialization has some important considerations:
            ...
            c. While wrapping the model with DDP, we set ``broadcast_buffers``
               to TRUE and broadcast module params and buffers from rank 0.
            d. The ``device_id`` param ensures that the DDP initialization happens on
               the correct device.
        """
        if self._is_rank_zero:
            log.info("DDP is enabled. Instantiating Model on CPU for Rank 0 ...")
        ...
        # Wrap the model with DDP. This will ensure that the model is replicated
    ...
    def save_checkpoint(self, epoch: int) -> None:
        ...
        cpu_state_dict = self._model.module.state_dict()
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    config.log_config(recipe_name="DDPFinetuneRecipeDistributed", cfg=cfg)

    recipe = DDPFinetuneRecipeDistributed(cfg=cfg)

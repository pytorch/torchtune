...
class PPOFullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def _setup_model(
        ...
        if enable_activation_checkpointing:
            ...
            utils.set_activation_checkpointing(value_model, auto_wrap_policy={modules.TransformerSelfAttentionLayer})
        ...
        if compile_model:
            ...
            # self._generate_trajectory_original = self.generate_trajectory
            # self.generate_trajectory = torch.compile(self.generate_trajectory, backend=backend)
            # policy_model.setup_caches()
            # self.compiled_generate_next_token = torch.compile(
            #     rlhf.generate_next_token_with_logits, fullgraph=False, backend="aot_eager"
            # )
    ...
    def generate_trajectory(self, input_ids: torch.Tensor) -> Trajectory:
        ...
        self._policy_model.setup_caches(batch_size=batch_size, dtype=self._dtype)
        del self._policy_model.causal_mask
        query_responses, logits = rlhf.generate_with_logits(
            ...
            rng=self._rng,
            custom_generate_next_token=self.compiled_generate_next_token,
        )

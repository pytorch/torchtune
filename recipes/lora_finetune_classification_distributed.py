...
class LoRAFinetunePromptClassificationRecipeDistributed(FTRecipeInterface):
    """
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self._validate_every_n_steps = cfg.get("validate_every_n_steps", None)
    ...
    def setup(self, cfg: DictConfig) -> None:
        ...
        ############################
        # Added 2 dataloaders so we can do validation
        # Ideally this would be handled by 1 dataloader
        ############################
    ...
    def train(self) -> None:
        ...
        ############################
        # Tried to add validation every n_steps, but I hit a deadlock from distributed setup
        # Seems like model forward or something is trying to sync the gpus and so doing validation
        # mid training hits deadlock. Validation currently only works at end of epoch
        ############################
        if self._validate_every_n_steps is None:
            self._validate_every_n_steps = self._steps_per_epoch
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                ############################
                # For classification, we pick the output for the eos token that
                # is added to the end of the prompt by dataloader.
                # Ideally, this part should be implemented as part of the TransformerDecoder class
                # directly for the model.
                ...
                # Pick the logits for the eos token for each item in the batch
                logits = logits[
                ...
                # for multi-class classification, we use BCEWithLogitsLoss so we don't transpose
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    ...
                    pbar.set_description(
                        f"{curr_epoch + 1}|{self.global_step}|train_loss: {loss_to_log}"
                    )
                    ...
                    ############################
                    # Free up memory
                    # This slows down training a bit (slower by ~5%), but I found it crucial for recovering
                    # reserved memory and preventing OOM when training.
                    ############################
                    gc.collect()
                ...
                # Run validation if step is every n step
                # if (
                #     (self.global_step > 0)
                #     and (self.global_step % self._validate_every_n_steps == 0)
                # ):
                #     # print(f"Rank {rank} is waiting for other gpus")
                #     # # make sure all gpus reach here for the step
                #     # torch.distributed.barrier()

                #     print(f"Running validation at step {self.global_step}")
                #     # run validation with rank 0 gpu
                #     if self._is_rank_zero:
                #         self.validation(curr_epoch)
                #         self._model.train()

                # print("Waiting for gpus")
                # # all gpus wait for the validation
                # torch.distributed.barrier()

            # validate at end of epoch if we didn't already do it as the last step
            # if (self.global_step % self._validate_every_n_steps != 0):
            self.validation(curr_epoch)
    ...
    ############################
    def validation(self, curr_epoch) -> None:
        """
        The validation loop.
        """
        ...
        for idx, batch in enumerate(self._val_dataloader):
            with torch.no_grad():
                ...
                pbar.update(1)
                pbar.set_description(
                    f"{curr_epoch + 1}|{self.global_step}|{idx+1}|val_batch_loss: {loss}|val_batch_accuracy: {accuracy}"
                )
...
def recipe_main(cfg: DictConfig) -> None:
    ...
    config.log_config(
        recipe_name="LoRAFinetunePromptClassificationRecipeDistributed", cfg=cfg
    )

    recipe = LoRAFinetunePromptClassificationRecipeDistributed(cfg=cfg)

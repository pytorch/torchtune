...
import pytest
import torch
from tests.test_utils import fixed_init_model, fixed_init_tensor  # assert_expected
...
class TestFlamingoVisionEncoder:
    ...
    def test_flamingo_vision_encoder(self, vision_transformer, transformer_config):
        ...
        expected_shape = (
            ...
            self.n_imgs,
            self.num_tiles,
            vision_transformer.vision_encoder.get_image_tokens_per_tile(),
            transformer_config["embed_dim_out"],
        )
        assert (
            output.shape == expected_shape
        ), f"Expected shape {expected_shape}, but got {output.shape}"

        # TODO (Felipe): add regression assertion
    ...
    def test_flamingo_vision_encoder_single_tile(self, transformer_config):
        ...
        expected_shape = (
            ...
            self.n_imgs,
            1,
            model_with_multiple_tiles.vision_encoder.get_image_tokens_per_tile(),
            transformer_config["embed_dim_out"],
        )
        assert (
            output.shape == expected_shape
    ...
    def test_flamingo_no_hidden_layers(self, vision_transformer, transformer_config):
        ...
        transformer_config = transformer_config.copy()
        transformer_config["out_indices"] = None
        ...
        expected_shape = (
            ...
            self.n_imgs,
            self.num_tiles,
            model_with_no_hidden.vision_encoder.get_image_tokens_per_tile(),
            transformer_config["embed_dim_out"],
        )
        assert (
            output.shape == expected_shape

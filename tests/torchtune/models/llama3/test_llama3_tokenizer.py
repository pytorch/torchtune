...
class TestLlama3Tokenizer:
    ...
    def test_all_tokens_work(
        ...
        special_tokens = list(range(128000, tokenizer.vocab_size))  # Llama 3 special tokens start at 128000

        all_tokens = normal_tokens + special_tokens

        for token in all_tokens:
            decoded = tokenizer.decode([token], skip_special_tokens=False, truncate_at_eos=False)
            assert isinstance(decoded, str)
        decoded = tokenizer.decode(all_tokens, skip_special_tokens=False, truncate_at_eos=False)

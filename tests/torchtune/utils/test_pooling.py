...
class TestPooling:
    ...
    def test_pool_sequence_logits_single_batch(self):
        ...
        output = pool_sequence_logits(tokens, logits, padding_token_idx=None)
        torch.testing.assert_close(output, expected_output)

    def test_pool_sequence_logits_fails_with_multi_batch_no_padding_idx(self):
        """
        Tests that an error is raised when no padding token index is provided for a multi-batch input.
        """
        tokens = torch.tensor([[1, 3, 4, 9], [4, 5, 6, 0], [1, 0, 0, 0], [0, 0, 0, 0]])
        logits = torch.tensor(
            [
        ...
        with pytest.raises(ValueError):
            pool_sequence_logits(tokens, logits, padding_token_idx=None)

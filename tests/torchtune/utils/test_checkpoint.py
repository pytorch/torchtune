...
class TestCheckpointUpdated:
    """
    This class should replace the one above, once load_checkpoint_updated replaces
    load_checkpoint and the current recipe is deprecated.
    """
    ...
    def _save_and_load(self, checkpoint, model, optimizer):
        with tempfile.NamedTemporaryFile() as f:
            ...
            if torch.distributed.is_initialized():
                ...
                file_name = f.name
            checkpoint = load_checkpoint_updated(
                ckpt_path=file_name,
    ...
    def _test_distributed_save_load(self) -> None:
        ...
        checkpoint = {"model": model, "optimizer": optim, "lr": 0.01}
        model_new, optim_new = self._get_model_and_optim(zero_model=True, fsdp=True)
        # Saves checkpoint, calls load_checkpoint_updated and loads model + optim states into new model/optim.
        self._save_and_load(checkpoint, model_new, optim_new)

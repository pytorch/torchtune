### Core issues:
  ...
  2) No support for weighted dataset:
    ...
    - We also support ConcatDataset, but its map style and there is no weighting;
  3) No support for on-the-fly data packing: It's done before training, taking a long time for large datasets;
...
### UX issues:
  ...
  5) Lack of dataloader args: args are scattered in the config. Important args are not exposed, e.g. num_workers, pin_memory, etc.
    ```yaml
    dataset:
  ...
  6) Different datasets have different arguments, because their message transforms are different.

### Principles:
  - Common API signatures for all datasets
  - Offload what we can to hf datasets methods directly
  - Less protagonism from our functions. E.g. config manipulations, instantiation, etc. (not the focus of this diff)
...
##########
# dataloader
# consolidate all dataloader args here, which are currently scattered
##########
dataloader:
...
#--------------------------------
#########
# dataset if the class is used directly, as in our current SFTDataset
#########
dataset:
...
#########
# Place for args common for all datasets that will be passed to the dataset constructor
# useful for multidataset. Used as cfg = dataset_defaults.update(dataset_cfg)
#########
dataset_defaults:
...
#########
# args used in the dataset setup. This is not dataset specific.
#########
dataset_setup:
    packing:
        _component_: torchtune.datasets.packing.SFTPacking
        max_seq_len: ${tokenizer.max_seq_len}
    multidataset_stopping_strategy: "first_exhausted" # "all_exhausted"
```
...
def alpaca_dataset(
    ...
    message_transform: optional[callable|dict],
    tokenizer: ModelTokenizer,
    ...
    *args, **kwargs
    ...
    # unify args
    if not message_transform or isinstance(message_transform, dict):
      # remove component key, since we are using alpaca_message_transform as default
    ...
    return HfIterableDataset(load_args, message_transform, tokenizer, recipe_transform, *args, **kwargs)
```

# Iterable dataset: Shared for all datasets and recipes (SFT, DPO, etc). Differences are in the transforms.
...
class HfIterableDataset(IterableDataset, Stateful):
    def __init__(
        ...
        *args, **kwargs
        ...
        #TODO: Maybe # shards should be based on dataset size, if we know it
        ...
        if filter_args:
            ...
            if function and not isinstance(function, Callable):
                raise ValueError(f"filter_args['function'] must be a callable. Found {type(function)}")
    ...
    def __iter__(self):
        ...
        # TODO: should we add try/except to handle/logerrors?
...
# Setup Data
Method in recipes/full_distributed.py
OR utility used in the recipe
...
#NOTE:  I have mixed feelings about passing multiple configDict to setup_data. This feels hard for the user to know what they should contain. On the other hand, i) setup_data doesnt need to make assumptions about the configs ii) we already do it currently. Alternative: use dataclassses
...
def setup_data(
    ...
    if setup_cfg.packing:
        ...
        ds = instantiate(data_setup_cfg.packing,
            dataset=ds,
    ...
    if collate_fn is None:
        collate_fn = "torchtune.data.padded_collate_packed" if packing else "torchtune.data.padded_collate_sft"
...
Options:
1. Make setup_data an utility, and have two utilities supporting the old and new config formats. After deprecation period, old utility is removed.

Pros: modularize it and remove from the recipe. Future changes will be easier to implement.
Cons: Big change in how we handle recipe utilities.

2. Create an adapter migrate_old_to_new_config:
Pros: Recipes still have method _setup_data exposing the logic
Cons: Hard to debug the migrated configs, edge cases not covered by the adapter, ConcatDataset is handled differently.

3. No migration. Old config with old recipe will break. Users need to update
their configs. No idea how this affects llamastack / startups / others.

**Implementation of option 1 (Make setup_data an utility)**

# torchtune/training/data_utils.py or similar location
```python
@deprecated

...
if _SUPPORTS_FLEX_ATTENTION:
    ...
    from torchtune.models.gemma2._attention_utils import (
        compile_friendly_flex_attention_with_score_and_block,
...
class FlexGemma2Attention(nn.Module):
    """
    Adapated from official Google Pytorch Implementation:
    https://github.com/google/gemma_pytorch/blob/80881c2e6e797ef1913a4a705d4b40394791cc58/gemma/model.py#L213
    to match torchtune style.
    ...
    def __init__(
        ...
        *,
        ...
        self.softcapping = softcapping
        if query_pre_attn_scalar is not None:
            # flex attention will always make the head_dim**-0.5 normalization so it should be included in scaling
            self.scaling = query_pre_attn_scalar**-0.5 / self.head_dim**-0.5
        else:
            self.scaling = None

        self.mask_mod = flex_causal_sliding_window(self.sliding_window_size)
        self.score_mod = flex_tanh_soft_capping_with_scaling(
    ...
    def forward(
        ...
        *,
        ...
        # https://pytorch.org/blog/flexattention/#q-when-should-we-recompute-the-blockmask
        ...
        output = compile_friendly_flex_attention_with_score_and_block(
            q, k, v, score_mod=self.score_mod, block_mask=block_mask
        )

...
class SiglipEncoderBlock(nn.Module):
  ...
  """
  def __init__(self, embedding_dim: int, num_attention_heads: int, head_dim: int, intermediate_size: int, layer_norm_eps: float,):
    super().__init__()
    self.self_attn = siglip_attention(
        embedding_dim, num_attention_heads, head_dim
    )
...
class SiglipVisionModel(nn.Module):
  ...
  def __init__(self, input_channels: int, embedding_dim: int, conv2d_patch_size: int, image_size: int, num_hidden_layers: int, num_attention_heads: int, head_dim: int, intermediate_size: int, layer_norm_eps: float, embedding_use_bias: bool):
    super().__init__()
    ...
    self.position_embedding = nn.Embedding(
        self.num_positions, embedding_dim
    )

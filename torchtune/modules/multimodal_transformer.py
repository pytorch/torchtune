...
class MMTransformerDecoder(nn.Module):
    """
    Multimodal Transformer Decoder, derived from the Llama2 architecture, with support for TransformerCrossAttentionLayer,
    so it can be used for multimodality.

    Args:
        tok_embeddings (nn.Embedding): PyTorch embedding layer, to be used to move
            tokens to an embedding space.
        layer (Union[nn.Module, nn.ModuleList]): Transformer Decoder layer or a nn.ModuleList of layers.
    ...
    def __init__(
        ...
        *,
        ...
        if num_layers is None:
            assert isinstance(
                layer, nn.ModuleList
            ), "If num_layers is undefined, it is assumed that a nn.ModuleList of layers is provided."
            layers = layer
        else:
